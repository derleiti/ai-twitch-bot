...
# Analyse des Bildes via Ollama
current_game = "Unbekanntes Spiel"
if latest_screenshot:
    try:
        description = get_vision_description(latest_screenshot)
        if description:
            # Extrahiere Spielname oder Szene aus dem Text
            def extract_title_from(desc):
                lines = desc.splitlines()
                for line in lines:
                    if "looks like" in line or "seems to be" in line:
                        return line
                return lines[0] if lines else "Unbekanntes Spiel"
            
            current_game = extract_title_from(description)
    except Exception as e:
        print(f"[!] Fehler bei Screenshot-Analyse: {e}")
