...
echo -e "${BLUE}=== Twitch-Ollama-Bot Updater v1.1 ===${NC}"
...
# Da wir in einer Debian-basierten Umgebung mit "externally-managed-environment" sind,
# verwenden wir apt oder erstellen eine virtuelle Umgebung

# Prüfe, ob python3-venv installiert ist
if ! dpkg -l | grep -q python3-venv; then
    echo -e "${YELLOW}Installiere python3-venv...${NC}"
    apt-get update
...
case $update_mode in
    ...
    2)
        ...
        # Lösche virtuelle Umgebung, falls vorhanden
        if [ -d "$VENV_DIR" ]; then
            rm -rf "$VENV_DIR"
            echo -e "${YELLOW}Virtuelle Umgebung gelöscht.${NC}"
        fi
        ;;
    3)
...
if [ ! -d "$VENV_DIR" ]; then
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}Virtuelle Umgebung erstellt: $VENV_DIR${NC}"
        "$VENV_DIR/bin/pip" install requests python-dotenv
    else
        echo -e "${RED}Fehler beim Erstellen der virtuellen Umgebung.${NC}"
        echo -e "${YELLOW}Versuche python3-venv zu installieren...${NC}"
        ...
        if [ $? -eq 0 ]; then
            ...
            echo -e "${RED}Konnte virtuelle Umgebung nicht erstellen.${NC}"
        fi
    fi
fi
