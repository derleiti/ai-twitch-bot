...
START_SCRIPT="$BASE_DIR/start-bot.sh"
...
echo -e "${BLUE}=== Twitch-Ollama-Bot Updater v1.0 ===${NC}"

# Prüfe, ob das Hauptverzeichnis existiert
if [ ! -d "$BASE_DIR" ]; then
    echo -e "${RED}Fehler: Das Verzeichnis $BASE_DIR existiert nicht!${NC}"
    read -p "Möchtest du das Verzeichnis erstellen? (j/n): " create_dir
...
# Prüfe, ob der Bot läuft und stoppe ihn
if [ -f "$PID_FILE" ]; then
    PID=$(cat "$PID_FILE")
    if ps -p "$PID" > /dev/null; then
        ...
        if ! ps -p "$PID" > /dev/null; then
            ...
            echo "$(date) - Bot wurde für Update gestoppt." >> "$LOG_FILE"
            rm "$PID_FILE"
        else
            echo -e "${RED}Bot konnte nicht beendet werden. Update abgebrochen.${NC}"
            exit 1
        fi
    else
...
# Erstelle ein Backup der aktuellen Skripte
echo -e "${YELLOW}Erstelle Backup der aktuellen Skripte...${NC}"

# Sichern des Bot-Skripts
if [ -f "$BOT_SCRIPT" ]; then
    cp "$BOT_SCRIPT" "$BACKUP_DIR/twitch-ollama-bot.py.$DATE_STAMP"
    echo -e "${GREEN}✓ Backup des Bot-Skripts erstellt${NC}"
fi

# Sichern des Start-Skripts
if [ -f "$START_SCRIPT" ]; then
    cp "$START_SCRIPT" "$BACKUP_DIR/start-bot.sh.$DATE_STAMP"
    echo -e "${GREEN}✓ Backup des Start-Skripts erstellt${NC}"
fi

# Sichern der Konfiguration
if [ -f "$CONFIG_FILE" ]; then
    cp "$CONFIG_FILE" "$BACKUP_DIR/.env.$DATE_STAMP"
    echo -e "${GREEN}✓ Backup der Konfigurationsdatei erstellt${NC}"
fi

# Sichern des Spielstands
if [ -f "$GAME_STATE_FILE" ]; then
    cp "$GAME_STATE_FILE" "$BACKUP_DIR/game_state.json.$DATE_STAMP"
    echo -e "${GREEN}✓ Backup des Spielstands erstellt${NC}"
fi
...
# Update-Optionen
echo -e "\n${BLUE}=== Update-Optionen ===${NC}"
echo -e "1) ${GREEN}Standard-Update${NC} - Aktualisiere den Bot auf die neueste Version"
echo -e "2) ${GREEN}Vollständige Neuinstallation${NC} - Löscht alle Bot-Dateien und installiert sie neu (Konfiguration bleibt erhalten)"
echo -e "3) ${GREEN}Nur Konfiguration behalten${NC} - Installiert den Bot neu und behält nur die .env-Datei"
echo -e "4) ${RED}Abbrechen${NC} - Update abbrechen"
echo -e ""
read -p "Wähle eine Option (1-4): " update_mode

case $update_mode in
    1)
        echo -e "${YELLOW}Führe Standard-Update durch...${NC}"
        ;;
    2)
        echo -e "${YELLOW}Führe vollständige Neuinstallation durch...${NC}"
        # Lösche alte Bot-Dateien, aber behalte Konfiguration und Backup
        if [ -f "$BOT_SCRIPT" ]; then
            rm "$BOT_SCRIPT"
            echo -e "${YELLOW}Bot-Skript gelöscht.${NC}"
        fi
        if [ -f "$START_SCRIPT" ]; then
            rm "$START_SCRIPT"
            echo -e "${YELLOW}Start-Skript gelöscht.${NC}"
        fi
        ;;
    3)
        echo -e "${YELLOW}Führe Neuinstallation durch, behalte nur Konfiguration...${NC}"
        # Lösche alle Dateien außer .env
        for file in "$BASE_DIR"/*; do
            if [ "$file" != "$CONFIG_FILE" ] && [ "$file" != "$BACKUP_DIR" ] && [ "$file" != "$BASE_DIR/backups" ]; then
                rm -rf "$file"
                echo -e "${YELLOW}Gelöscht: $file${NC}"
            fi
        done
        ;;
    4)
        ...
        ;;
    *)
        echo -e "${RED}Ungültige Option. Update abgebrochen.${NC}"
        exit 1
        ;;
esac

# Erstelle oder aktualisiere das Bot-Skript
echo -e "${YELLOW}Aktualisiere Bot-Skript...${NC}"
cat > "$BOT_SCRIPT" << 'EOL'
#!/usr/bin/env python3
...
# Erstelle oder aktualisiere das Start-Skript
echo -e "${YELLOW}Aktualisiere Start-Skript...${NC}"
...
# Erstelle/aktualisiere .env Konfigurationsdatei, wenn sie nicht existiert
if [ ! -f "$CONFIG_FILE" ]; then
    echo -e "${YELLOW}Erstelle Konfigurationsdatei .env...${NC}"
    cat > "$CONFIG_FILE" << 'EOL'
BOT_USERNAME=derleiti
OAUTH_TOKEN=oauth:gbsmp9yzw1mvar983b76g6n2oyieyr
CHANNEL=#derleiti
BOT_NAME=zephyr
OLLAMA_URL=http://localhost:11434/api/generate
OLLAMA_MODEL=zephyr
AUTO_JOKE_INTERVAL=180
AUTO_COMMENT_INTERVAL=240
AUTO_SCENE_COMMENT_INTERVAL=300
COMMAND_REMINDER_INTERVAL=600
OLLAMA_TIMEOUT=30
DEBUG_LEVEL=1
EOL
    echo -e "${GREEN}Konfigurationsdatei erstellt. Bitte passe sie bei Bedarf an.${NC}"
else
    echo -e "${GREEN}Konfigurationsdatei .env existiert bereits und wurde nicht überschrieben.${NC}"
fi

# Erstelle/aktualisiere game_state.json, wenn sie nicht existiert
if [ ! -f "$GAME_STATE_FILE" ]; then
    echo -e "${YELLOW}Erstelle Spielstand-Datei game_state.json...${NC}"
    cat > "$GAME_STATE_FILE" << 'EOL'
{
  "spiel": "Unbekannt",
  "ort": "Unbekannt",
  "tode": 0,
...
EOL
    echo -e "${GREEN}Spielstand-Datei erstellt.${NC}"
else
    echo -e "${GREEN}Spielstand-Datei game_state.json existiert bereits und wurde nicht überschrieben.${NC}"
fi

# Erstelle README.md mit Dokumentation
echo -e "${YELLOW}Erstelle Dokumentation...${NC}"
cat > "$BASE_DIR/README.md" << 'EOL'
# Twitch-Ollama-Bot

Ein Twitch-Chatbot, der Ollama verwendet, um KI-gestützte Interaktionen zu bieten.

## Features

- 🎭 Automatische Witze im Chat
- 🎮 Kommentare zum aktuellen Spiel
- 👁️ Beschreibungen und Kommentare zu Szenen/Bildern
- 💬 Fragen-Beantwortung mit Ollama-Integration
...
- `!info` - Zeigt Informationen zum aktuellen Spiel
- `!stats` - Zeigt Spielstatistiken an
- `!bild` oder `!scene` - Kommentiert das aktuelle Bild/die aktuelle Szene
- `!spiel NAME` - Ändert den Namen des aktuellen Spiels
- `!ort NAME` - Ändert den aktuellen Ort im Spiel
...
2. Installiere die erforderlichen Pakete: `pip install requests python-dotenv`
3. Installiere Ollama von [https://ollama.ai](https://ollama.ai)
4. Lade das Zephyr-Modell herunter: `ollama pull zephyr`
5. Konfiguriere die Datei `.env` mit deinen Twitch-Anmeldedaten
6. Starte den Bot mit `./start-bot.sh`

## Konfiguration

Die Konfiguration erfolgt in der Datei `.env` mit folgenden Parametern:

```
BOT_USERNAME=dein_twitch_username
OAUTH_TOKEN=oauth:dein_token
CHANNEL=#dein_kanal
BOT_NAME=zephyr
OLLAMA_URL=http://localhost:11434/api/generate
OLLAMA_MODEL=zephyr
...
Der Bot kann als systemd-Service eingerichtet werden, um automatisch beim Systemstart zu starten. Das Setup-Skript bietet diese Option an.

## Troubleshooting

- Wenn der Bot keine Verbindung zu Twitch herstellen kann, überprüfe deine Anmeldedaten in der `.env`-Datei.
- Wenn der Bot keine Antworten von Ollama erhält, stelle sicher, dass Ollama läuft und das Zephyr-Modell installiert ist.
- Überprüfe die Logdatei `twitch-ollama-bot.log` für detaillierte Fehlerinformationen.
EOL

echo -e "${GREEN}Update abgeschlossen! Du kannst den Bot jetzt mit folgendem Befehl starten:${NC}"
echo -e "${YELLOW}$START_SCRIPT${NC}"

# Frage, ob der Bot direkt gestartet werden soll
echo -e "\n${BLUE}Möchtest du den Bot jetzt starten? (j/n): ${NC}"
read start_bot

if [ "$start_bot" = "j" ] || [ "$start_bot" = "J" ]; then
    exec "$START_SCRIPT"
else
    echo -e "${YELLOW}Bot wurde nicht gestartet. Du kannst ihn später mit '$START_SCRIPT' starten.${NC}"
fi
